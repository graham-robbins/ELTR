...
\end{equation}
The normalized spread $\tilde{s}_t$ facilitates comparison across contracts with different price levels. Intuitively, it measures the cost of crossing the market relative to the current price.
...
\end{equation}
where $\hat{s}^v_t$ is the rolling standard deviation of volume over the same window $W$, computed with a small-sample safeguard. Intuitively, $z^v_t$ measures how many standard deviations current trading volume lies above its recent baseline.
